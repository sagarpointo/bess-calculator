 ...
 */
export function calculate({ kva, hours, dieselPrice, gridTariff, solar, tenureYears = 5 }) {
  const C = CONFIG
  ...
  const energySavePct = dgCostMonth > 0 ? (energySaveMonth / dgCostMonth) * 100 : 0
  ...
  // ---- Payback (months to recover full customer price from total savings) ----
  const paybackMonths = totalSaveMonth > 0 ? Math.ceil(customerPrice / totalSaveMonth) : 0
  ...
  return {
    kw, kwhPerDay,
    dgCostDay, dgCostMonth, dgCostYear, dgUnitCost,
    bessCostDay, bessCostMonth, bessCostYear, bessUnitCost,
    energySaveMonth, energySaveYear, energySavePct,
    dgMaintYear: C.DG_MAINT_YR, dgOperatorYear: C.DG_OPERATOR_YR,
    totalSaveMonth, totalSaveYear,
    systemPrice, processingFee, gst, gstRate, customerPrice,
    emi, totalPayable, totalInterest, netGainMonth, tenureMonths,
    paybackMonths,
    co2TonnesYear, trees,
  }
}
